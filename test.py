...
async def test_get_sorted_recipes(client, test_db):
    ...
    async with test_db() as session:
        recipe1 = models.Recipe(dish_name='Eggs', views=10, cook_time=30, ingredients='...', description='.....')
        recipe2 = models.Recipe(dish_name='Soup', views=50, cook_time=60, ingredients='...', description='.....')
        recipe3 = models.Recipe(dish_name='Pie', views=10, cook_time=5, ingredients='...', description='.....')
...
async def test_get_recipe_by_id(client, test_db):
    async with test_db() as session:
        recipe1 = models.Recipe(dish_name='Eggs', cook_time=5, ingredients='...', description='.....')
        session.add(recipe1)
        await session.commit()
...
async def test_post_recipe(client, test_db):
    ...
    async with test_db() as session:
        ...
        res = await session.execute(select(models.Recipe).where(models.Recipe.id == data['id']))
